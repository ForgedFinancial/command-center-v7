// Proxy: /api/taskboard/* → VPS sync server
var ALLOWED_ORIGINS = [
...
export async function onRequest(context) {
  ...
  const target = `https://api.forgedfinancial.us/api/taskboard/${path}${originalUrl.search}`;
  ...
  try {
    ...
    return jsonResponse({ error: "Proxy error", detail: err.message }, 502, corsHeaders);
  }
}
