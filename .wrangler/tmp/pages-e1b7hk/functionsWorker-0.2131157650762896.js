...
async function handleRequest(request, env) {
  ...
  try {
    if (path === "/health") {
      return jsonResponse({ status: "healthy", service: "yncrm-api", timestamp: (/* @__PURE__ */ new Date()).toISOString() }, 200, corsHeaders);
    }
...
async function createJWT(agent, env) {
  ...
  const encode = /* @__PURE__ */ __name((obj) => btoa(JSON.stringify(obj)).replace(/=/g, ""), "encode");
...
__name(handleAdminReset, "handleAdminReset");
...
__name(reorderPipelines, "reorderPipelines");
...
__name(createStage, "createStage");
async function updateStage(id, request, env, corsHeaders) {
  ...
  for (const f of ["required_fields", "tags_on_entry", "automations"]) {
    if (data[f] !== void 0) {
      updates.push(`${f} = ?`);
      values.push(safeStringify(data[f], []));
    }
  }
  if (updates.length > 0) {
    updates.push('updated_at = datetime("now")');
...
__name(updateStage, "updateStage");
...
__name(deleteStage, "deleteStage");
...
__name(reorderStages, "reorderStages");
...
__name(transferLead, "transferLead");
...
__name(getLeadHistory, "getLeadHistory");
async function listPipelineHistory(url, env, corsHeaders) {
  ...
  if (pipelineId) {
    query += " AND (to_pipeline = ? OR from_pipeline = ?)";
...
__name(listPipelineHistory, "listPipelineHistory");
...
__name(getLeadTags, "getLeadTags");
...
__name(addLeadTags, "addLeadTags");
...
__name(removeLeadTag, "removeLeadTag");
...
__name(getMetricSettings, "getMetricSettings");
...
__name(setMetricLevel, "setMetricLevel");
...
__name(bulkSetMetrics, "bulkSetMetrics");
...
__name(listSmsTemplates, "listSmsTemplates");
async function updateSmsTemplate(id, request, env, corsHeaders) {
  ...
  if (data.variables !== void 0) {
    ...
    values.push(safeStringify(data.variables, []));
  }
  if (updates.length > 0) {
...
__name(updateSmsTemplate, "updateSmsTemplate");
...
__name(resetSmsTemplate, "resetSmsTemplate");
...
__name(listTimerConfigs, "listTimerConfigs");
...
__name(updateTimerConfig, "updateTimerConfig");
...
__name(getNotificationPrefs, "getNotificationPrefs");
...
__name(updateNotificationPrefs, "updateNotificationPrefs");
...
__name(listCallRecordings, "listCallRecordings");
...
__name(getCallRecording, "getCallRecording");
...
__name(handleTwilioRecordingWebhook, "handleTwilioRecordingWebhook");
...
__name(listVoicemailDrops, "listVoicemailDrops");
...
__name(createVoicemailDrop, "createVoicemailDrop");
...
__name(deleteVoicemailDrop, "deleteVoicemailDrop");
...
__name(listVoicemails, "listVoicemails");
...
async function createLead(request, agent, env, corsHeaders) {
  ...
  `).bind(
    ...
    data.lead_type || null,
    data.value || 0,
    safeStringify(data.tags, []),
    safeStringify(data.custom_fields, {}),
    data.notes || null,
    data.state || null,
    data.carrier || null,
    data.premium || 0,
    ...
    data.face_amount || 0,
    data.draft_date || null,
    data.payment_method || null,
    data.beneficiary || null,
    data.beneficiary_relation || null,
    data.beneficiary2 || null,
    data.beneficiary2_relation || null,
    data.dob || null,
    data.ssn || null,
    data.bank_name || null,
    data.routing || null,
...
async function updateLead(id, request, agent, env, corsHeaders) {
  ...
  if (data.tags !== void 0) {
    updates.push("tags = ?");
    values.push(safeStringify(data.tags, []));
  }
  if (data.custom_fields !== void 0) {
    updates.push("custom_fields = ?");
    values.push(safeStringify(data.custom_fields, {}));
  }
  if (updates.length > 0) {
    updates.push('updated_at = datetime("now")');
...
__name(bulkDeleteLeads, "bulkDeleteLeads");
...
__name(importLeads, "importLeads");
...
async function updateEvent(id, request, agent, env, corsHeaders) {
  ...
  if (data.dirty !== void 0) {
    updates.push("dirty = ?");
    values.push(data.dirty ? 1 : 0);
  }
  updates.push('updated_at = datetime("now")');
  values.push(id, agent.id);
...
async function updateAutomation(id, request, agent, env, corsHeaders) {
  ...
  if (data.trigger_type !== void 0) {
    updates.push("trigger_type = ?");
    values.push(data.trigger_type);
  }
  if (data.trigger_config !== void 0) {
...
__name(updateAutomation, "updateAutomation");
...
__name(deleteAutomation, "deleteAutomation");
...
__name(listDripSources, "listDripSources");
...
__name(createDripSource, "createDripSource");
async function updateDripSource(id, request, agent, env, corsHeaders) {
  const data = await request.json();
  const updates = [];
  const values = [];
  if (data.sheet_url !== void 0) {
    updates.push("sheet_url = ?");
    values.push(data.sheet_url);
  }
  if (data.poll_interval !== void 0) {
    updates.push("poll_interval = ?");
    values.push(data.poll_interval);
  }
  if (data.enabled !== void 0) {
    updates.push("enabled = ?");
    values.push(data.enabled ? 1 : 0);
...
__name(updateDripSource, "updateDripSource");
...
__name(deleteDripSource, "deleteDripSource");
...
__name(listActivities, "listActivities");

// ../.wrangler/tmp/pages-e1b7hk/functionsRoutes-0.14193281472545483.mjs
var routes = [
  {
