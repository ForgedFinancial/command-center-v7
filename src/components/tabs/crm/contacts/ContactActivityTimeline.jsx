...
export default function ContactActivityTimeline({ contactId }) {
  ...
  if (loading) {
    return <div style={{ padding: '24px', textAlign: 'center', color: 'var(--theme-text-secondary)', fontSize: '12px' }}>Loading activity...</div>
  }
      ...
      {activities.map((item, i) => (
        <div key={item.id || i} style={{ position: 'relative', marginBottom: '16px', paddingLeft: '8px' }}>
          ...
          <div style={{
            ...
            background: 'var(--theme-accent-muted)', border: '2px solid var(--theme-accent)',
