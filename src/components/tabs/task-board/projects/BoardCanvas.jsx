        ...
        {!!alignmentGuides?.length && (
          <svg style={{ position: 'absolute', inset: 0, overflow: 'visible', pointerEvents: 'none' }}>
            {alignmentGuides.map((guide, idx) => (
              guide.axis === 'x'
                ? <line key={`gx-${idx}`} x1={guide.value} y1={-10000} x2={guide.value} y2={10000} stroke="#06b6d4" strokeWidth="1" />
                : <line key={`gy-${idx}`} x1={-10000} y1={guide.value} x2={10000} y2={guide.value} stroke="#06b6d4" strokeWidth="1" />
            ))}
          </svg>
        )}
        {[...items].sort((a, b) => (a.type === 'frame' ? -1 : 0) - (b.type === 'frame' ? -1 : 0)).map((item) => (
          <BoardItem key={item.id} item={item} isSelected={selectedIds.has(item.id)} isFrameHover={item.type === 'frame' && highlightFrameId === item.id} onPointerDown={(event) => onItemPointerDown(event, item.id)} onContextMenu={(event) => onItemContextMenu(event, item.id)} onContentChange={(content) => onItemContentChange(item.id, content)} onAiAction={onAiAction} onDocumentExpand={onDocumentExpand} onStartConnector={(event, side) => onStartConnector?.(event, item.id, side)} />
        ))}
